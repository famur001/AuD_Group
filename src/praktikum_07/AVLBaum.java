...
// Methoden zum Suchen
	...
	public boolean suchen(final T daten) {
		AVLKnoten<T> baum = wurzel;
		while (baum != null) {
			int vergleich = daten.compareTo(baum.getDaten());
			if (vergleich == 0)
				return true;
			else if (vergleich == 1) {
				baum = baum.getKnotenRechts();
			} else {
				baum = baum.getKnotenLinks();
			}
		}
		return false;
	}
...
// Methode zum Traversieren
	...
	public String traversierePreOrder() {
		...
		while( !stack.isEmpty() ) {
			...
			AVLKnoten<T> element = stack.pop();
			erg += element.getDaten();
			if( element.getKnotenRechts() != null ) {
				stack.push(element.getKnotenRechts());
			}
			if( element.getKnotenLinks() != null ) {
				stack.push(element.getKnotenLinks());
			}
