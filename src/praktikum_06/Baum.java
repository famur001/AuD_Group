...
// Methoden zum Traversieren
	...
	private String traversierePreOrder(final Knoten<T> einKnoten)
	{
		...
		erg += einKnoten.getDaten(); // Konten k
		if( einKnoten.getKnotenLinks() != null ) { // Linker Teilbaum
			erg += traversierePreOrder(einKnoten.getKnotenLinks());
		}
		if( einKnoten.getKnotenRechts() != null ) { // Rechter Teilbaum
...
// Methoden zur Bestimmung der H�he
	...
	private int hoeheRek(final Knoten<T> einKnoten)
	{
		...
		int links = 1, rechts = 1;
		if( einKnoten.getKnotenLinks() != null ) {
			links = 1+hoeheRek(einKnoten.getKnotenLinks());
		}
		if( einKnoten.getKnotenRechts() != null ) {
			rechts = 1+hoeheRek(einKnoten.getKnotenRechts());
		}
