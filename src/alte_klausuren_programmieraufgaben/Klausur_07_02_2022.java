...
public class Klausur_07_02_2022 {
	...
	boolean istBinär(BKnoten knoten) {
		if (knoten == null) return true; // leerer Baum ist binär
		if (knoten.kinder.length > 2){ // Wenn ein Knoten mehr als 2 Kinder besitzt, kann der Baum nicht mehr binär sein
			return false;
		}
		if(knoten.kinder[0] != null) istBinär(knoten.kinder[0]); // Gibt es ein linkes Kind? Wenn ja besuche es rekusiv
		if(knoten.kinder[1] != null) istBinär(knoten.kinder[1]); // Gibt es ein rechtes Kind? Wenn ja besuche es rekusiv
